...
public class Cart {
	...
	public synchronized void addItem(Product product, int amount) {
		...
		if (product == null || amount <= 0) {
			logger.error("Попытка добавить товар с некорректными параметрами: product={}, amount={}", product, amount);
	...
	public synchronized void removeItem(Product product, int amount) {
		...
		if (product == null || amount <= 0) {
			logger.error("Попытка удалить товар с некорректными параметрами: product={}, amount={}", product, amount);
			throw new IllegalArgumentException("Товар не найден или кол-во неверное");
		}
		...
		if (goods.containsKey(product)) {
			...
			if (currentAmount <= amount) {
				...
				logger.info("Кол-во товара успешно убавилось на {}. Новое кол-во для {}: {}", amount, product.getName(), currentAmount - amount);
